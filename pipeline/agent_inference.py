...
{{
  ...
  "hull_box": [rx1, ry1, rx2, ry2]
}}

hull_box 说明：
- 给出弦号文字在图像中的精确边界框（bounding box）
- 坐标为相对值，范围 0.0~1.0，表示占图像宽高的比例
- [rx1, ry1] 是左上角，[rx2, ry2] 是右下角
- 如果完全看不到弦号文字，返回空数组 []
- 只框选弦号编号文字本身，不要框选整艘船"""
...
hull_box：弦号文字的边界框（相对坐标 0.0~1.0），无弦号则返回 []"""
...
class AgentInference:
        ...
        try:
            ...
            if match:
                try:
                    ...
                    return {"hull_number": "", "description": content[:200], "hull_box": None}
            else:
    ...
    def _infer_single_inner(
        ...
        logger.info(
            "推理完成 (track=%d, frame=%d): 弦号=%s, 描述=%s, hull_box=%s",
            track_id, frame_id,
