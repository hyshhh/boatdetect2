...
class ShipPipeline:
    ...
    def _run_three_step_chain(
        ...
        self._log_agent_trace(
            ...
            track_id=track_id,
            frame_id=frame_id,
            content=f"弦号={hull_number or '(无)'} 描述={description[:40] if description else '(无)'} hull_box={hull_box or '(无)'}",
        )

        if not hull_number and not description:
            return AgentResult(answer="VLM 未返回结果", hull_box=hull_box)

        return self._local_lookup_retrieve(hull_number, description, track_id=track_id, frame_id=frame_id, hull_box=hull_box)
    ...
    def _run_agent_chain(self, crop: np.ndarray, track_id: int = 0, frame_id: int = 0) -> AgentResult:
        ...
        self._log_agent_trace(
            ...
            track_id=track_id,
            frame_id=frame_id,
            content=f"VLM预识别: 弦号={hull_number or '(无)'} 描述={description[:50] if description else '(无)'} hull_box={hull_box or '(无)'}",
        )
        ...
        # 传递 hull_box（VLM 产出，Agent 不涉及坐标）
